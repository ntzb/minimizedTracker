...
class MonitorChanger
{
    public static void SetAsPrimaryMonitor(uint id)
    {
        ...
        NativeMethods.ChangeDisplaySettingsEx(
            ...
            (IntPtr)null,
            (
                ChangeDisplaySettingsFlags.CDS_SET_PRIMARY
        ...
        for (
            uint otherid = 0;
            NativeMethods.EnumDisplayDevices(null, otherid, ref device, 0);
            otherid++
        )
        {
            if (
                device.StateFlags.HasFlag(DisplayDeviceStateFlags.AttachedToDesktop)
                && otherid != id
            )
            {
                device.cb = Marshal.SizeOf(device);
    ...
    public static uint GetMonitor(bool wantPrimary)
    {
        uint retVal = 0;
        uint monitorIndex = 0;
        while (true)
        {
            ...
            bool displayExists = NativeMethods.EnumDisplayDevices(
                null,
                monitorIndex,
                ref device,
                0
            ...
            if (!displayExists)
            {
                Console.WriteLine("could not find a valid primary display");
                break;
            }

            DisplayDeviceStateFlags wantedFlags = DisplayDeviceStateFlags.AttachedToDesktop;
            if (wantPrimary)
            {
                wantedFlags |= DisplayDeviceStateFlags.PrimaryDevice;
            }
            if (device.StateFlags.HasFlag(wantedFlags))
            {
                retVal = monitorIndex;
                break;
            }
    ...
    public static void SetDpiAwareness()
    {
        try
        {
            ...
            {
                NativeMethods.SetProcessDpiAwareness(ProcessDPIAwareness.ProcessPerMonitorDPIAware);
            }
        }
